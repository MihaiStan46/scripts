...
variable "billing_account" {
  description = "003DF8-786EF1-393A63"
  type        = string
}

resource "google_project" "project" {
  name            = "wave-buh-temp-mihailstan"
  project_id      = "wave-buh-temp-mihailstan-${random_id.project_suffix.dec}"
...
resource "google_compute_backend_bucket" "default" {
  name        = "reprodstan-bucket-backend"
  project     = google_project.project.project_id
...
resource "google_compute_url_map" "https_url_map" {
  name        = "reprodstan-com-https-url-map"
  project     = google_project.project.project_id
...
resource "google_compute_target_https_proxy" "https_proxy" {
  name             = "reprodstan-com-https-proxy"
  project          = google_project.project.project_id
...
resource "google_compute_url_map" "http_url_map" {
  name    = "reprodstan-com-http-url-map"
  project = google_project.project.project_id
...
resource "google_compute_target_http_proxy" "http_proxy" {
  name    = "reprodstan-com-http-proxy"
  project = google_project.project.project_id
